...
const TOKEN_SYMBOL = process.env.TOKEN_SYMBOL || 'AMENS';
...
function toPublicActivityRow(s, price) {
  ...
  switch (s.name) {
    ...
    case 'buy':
      amountSol = d.solSpent ?? null;
      amountTokens = d.tokensBought ?? null;
      break;
    case 'airdrop':
      amountTokens = d.tokensSent ?? null;
      status = d.failed ? 'failed' : 'completed';
      break;
    case 'burn':
      amountTokens = d.tokensBurned ?? null;
      break;
    default:
      break;
  }
...
// Headline numbers for the frontend: $AMENS bought back and sent to the
// reward wallet. byMint is keyed by reward_mint (repo.getAirdropTotals):
// { sends, totalUi, holders }.
function toPublicSummary({ stats, byMint, price, tokenMint, rewardWallet, marketCapUsd = null }) {
  const z = { totalUi: 0, holders: 0, sends: 0 };
  const bought = byMint[tokenMint] || z;
  ...
  return {
    ...
    // $AMENS bought back and delivered to the reward wallet
    buybackDistributed: bought.totalUi,
    distributions: bought.sends,
    // $AMENS burned on-chain across all cycles
    burned: stats.total_tokens_burned || 0,
    recipient: rewardWallet ?? null,
  };
}
